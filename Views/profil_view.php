...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
                            ...
                            <div class="card card-primary card-outline">
                                <div class="card-body box-profile">
                                    <div class="text-center">
                                        <img class="profile-user-img img-fluid img-circle" src="Assets/Images/<?php echo htmlspecialchars($userFoto); ?>" alt="Profil Pengguna">
                                    </div>
                                    <form id="editProfil" method="POST" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="nama">Nama</label>
                                            <input type="text" class="form-control" name="nama" id="nama" value="<?php echo htmlspecialchars($dataUser['nama']); ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="email">Email</label>
                                            <input type="text" class="form-control" name="email" id="email" value="<?php echo htmlspecialchars($dataUser['email']); ?>">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <div class="custom-file">
                                                    ...
                                                    <label class="custom-file-label text-gray" id="file-label" for="foto">Pilih Foto</label>
                                                </div>
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-primary">
                                            <li class="fa fa-print"></li> Simpan
                                        </button>
                                    </form>
                                </div>
                                <!-- /.card-body -->
    ...
    <script>
        ...
        $("#editProfil").on("submit", function(e) {
            ...
            var formData = new FormData(this); // Gunakan FormData untuk handle file upload

            $.ajax({
                type: "POST",
                url: "dashboard_profil.php",
                data: formData,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.newFoto) {
                            $('.profile-user-img, .img-circle.elevation-2').attr('src', 'Assets/Images/' + response.newFoto);
                            // Update label nama file di form
                            $('#file-label').text("Pilih Foto");
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Gagal mengubah profil: ' + (xhr.responseJSON?.message || xhr.responseText || error),
                        icon: 'error',
                        confirmButtonText: 'Tutup'
                    });
                }

...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="button">
                                        <a href="dashboard_tugas.php" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
                                    </div>
                                </div>
                                <form id="editTugasForm" action="editTugas.php?id=<?php echo htmlspecialchars($dataTugas['id']); ?>" method="post">
                                    <input type="hidden" name="id" id="id" value="<?php echo htmlspecialchars($dataTugas['id']); ?>">
                                    <div class="card-body">
                                            ...
                                            <select class="form-control" name="id_kategori" id="id_kategori" required>
                                                ...
                                                <?php if ($dataKategori) : ?>
                                                    <?php foreach ($dataKategori as $kat) : ?>
                                                        <option value="<?= htmlspecialchars($kat['id']) ?>" <?= (isset($dataTugas['id_kategori']) && $dataTugas['id_kategori'] == $kat['id']) ? 'selected' : '' ?>>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="judul" id="judul" value="<?php echo htmlspecialchars($dataTugas['judul']); ?>">
                                        </div>
                                        <div class="form-group">
                                            <label>Deskripsi</label>
                                            <textarea class="form-control" name="deskripsi" id="deskripsi" rows="3"><?php echo htmlspecialchars($dataTugas['deskripsi']); ?></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label>Deadline:</label>
                                            <div class="input-group date" id="reservationdatetime" data-target-input="nearest">
                                                <input type="text" name="deadline" id="deadline" class="form-control datetimepicker-input" data-target="#reservationdatetime" value="<?php echo htmlspecialchars($dataTugas['deadline']); ?>">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="status" id="status">
                                                ...
                                                <option value="belum" <?= (isset($dataTugas['status']) && $dataTugas['status'] === 'belum') ? 'selected' : '' ?>>Belum</option>
                                                <option value="proses" <?= (isset($dataTugas['status']) && $dataTugas['status'] === 'proses') ? 'selected' : '' ?>>Proses</option>
                                                <option value="selesai" <?= (isset($dataTugas['status']) && $dataTugas['status'] === 'selesai') ? 'selected' : '' ?>>Selesai</option>
                                            </select>
                                        </div>
                                    </div>
                                    <!-- /.card-body -->
    ...
    <script>
        $("#editTugasForm").on("submit", function(e) {
            e.preventDefault(); // Mencegah form dari pengiriman default

            $.ajax({
                type: "POST",
                url: "editTugas.php?id=" + $("#id").val(), // Jika menggunakan ID
                data: $(this).serialize(),
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location = 'dashboard_tugas.php';
                            }
                        });
                    } else {
                        Swal.fire({
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Gagal mengubah tugas: ' + xhr.responseText,
                        icon: 'error',
                        confirmButtonText: 'Tutup'
                    });
                }

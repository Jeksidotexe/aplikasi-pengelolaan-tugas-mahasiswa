...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Edit Prioritas</h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
                                <li class="breadcrumb-item active">Edit Prioritas</li>
                            </ol>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="button">
                                        <a href="dashboard_prioritas.php" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
                                    </div>
                                </div>
                                <form id="editPrioritasForm" action="editPrioritas.php?id=<?php echo htmlspecialchars($dataPrioritas['id']); ?>" method="post">
                                    <input type="hidden" name="id" id="id" value="<?php echo htmlspecialchars($dataPrioritas['id']); ?>">
                                    <div class="card-body">
                                            ...
                                            <select class="form-control" name="id_tugas" id="id_tugas" required>
                                                ...
                                                <?php if ($dataTugas) : ?>
                                                    <?php foreach ($dataTugas as $t) : ?>
                                                        <option value="<?= htmlspecialchars($t['id']) ?>" <?= (isset($dataPrioritas['id_tugas']) && $dataPrioritas['id_tugas'] == $t['id']) ? 'selected' : '' ?>>
                                                            <?= htmlspecialchars($t['judul']) ?></option>
                                                    <?php endforeach; ?>
                                                <?php else : ?>
                                                    <option value="">Tidak ada tugas yang tersedia</option>
                                                <?php endif; ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Status</label>
                                            <select class="form-control" name="level_prioritas" id="level_prioritas">
                                                <option value="">-- Pilih Status --</option>
                                                <option value="rendah" <?= (isset($dataPrioritas['level_prioritas']) && $dataPrioritas['level_prioritas'] === 'rendah') ? 'selected' : '' ?>>Rendah</option>
                                                <option value="sedang" <?= (isset($dataPrioritas['level_prioritas']) && $dataPrioritas['level_prioritas'] === 'sedang') ? 'selected' : '' ?>>Sedang</option>
                                                <option value="tinggi" <?= (isset($dataPrioritas['level_prioritas']) && $dataPrioritas['level_prioritas'] === 'tinggi') ? 'selected' : '' ?>>Tinggi</option>
                                            </select>
                                        </div>
                                    </div>
                                    <!-- /.card-body -->
    ...
    <script>
        $("#editPrioritasForm").on("submit", function(e) {
            e.preventDefault(); // Mencegah form dari pengiriman default

            $.ajax({
                type: "POST",
                url: "editPrioritas.php?id=" + $("#id").val(), // Jika menggunakan ID
                data: $(this).serialize(),
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location = 'dashboard_prioritas.php';
                            }
                        });
                    } else {
                        Swal.fire({
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Gagal mengubah prioritas: ' + xhr.responseText,
                        icon: 'error',
                        confirmButtonText: 'Tutup'
                    });
                }

...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="button">
                                        <a href="dashboard_kategori.php" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
                                    </div>
                                </div>
                                <?php if (isset($error)): ?>
                                    <p class="error"><?php echo htmlspecialchars($error); ?></p>
                                <?php endif; ?>
                                <form id="editKategoriForm" action="editKategori.php?id=<?php echo htmlspecialchars($dataKategori['id']); ?>" method="post">
                                    <input type="hidden" name="id" id="id" value="<?php echo htmlspecialchars($dataKategori['id']); ?>">
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="nama_kategori">Nama Kategori</label>
                                            <input type="text" class="form-control" name="nama_kategori" id="nama_kategori" value="<?php echo htmlspecialchars($dataKategori['nama_kategori']); ?>">
                                        </div>
                                    </div>
                                    <!-- /.card-body -->
    ...
    <script>
        $("#editKategoriForm").on("submit", function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "editKategori.php?id=" + $("#id").val(), // Jika menggunakan ID
                data: $(this).serialize(),
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location = 'dashboard_kategori.php';
                            }
                        });
                    } else {
                        Swal.fire({
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Gagal mengubah kategori: ' + xhr.responseText,
                        icon: 'error',
                        confirmButtonText: 'Tutup'
                    });
                }

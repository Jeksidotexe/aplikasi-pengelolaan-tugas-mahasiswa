...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="button">
                                        <a href="dashboard_komentar.php" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
                                    </div>
                                </div>
                                <form id="editKomentarForm" action="editKomentar.php?id=<?php echo htmlspecialchars($dataKomentar['id']); ?>" method="post">
                                    <input type="hidden" name="id" id="id" value="<?php echo htmlspecialchars($dataKomentar['id']); ?>">
                                    <div class="card-body">
                                            ...
                                            <select class="form-control" name="id_tugas" id="id_tugas" required>
                                                ...
                                                <?php if ($dataTugas) : ?>
                                                    <?php foreach ($dataTugas as $t) : ?>
                                                        <option value="<?= htmlspecialchars($t['id']) ?>" <?= (isset($dataKomentar['id_tugas']) && $dataKomentar['id_tugas'] == $t['id']) ? 'selected' : '' ?>>
                                        ...
                                        <div class="form-group">
                                            <label>Isi</label>
                                            <textarea class="form-control" name="isi" id="isi" rows="3"><?php echo htmlspecialchars($dataKomentar['isi']); ?></textarea>
                                        </div>
                                    </div>
                                    <!-- /.card-body -->
    ...
    <script>
        $("#editKomentarForm").on("submit", function(e) {
            e.preventDefault(); // Mencegah form dari pengiriman default

            $.ajax({
                type: "POST",
                url: "editKomentar.php?id=" + $("#id").val(), // Jika menggunakan ID
                data: $(this).serialize(),
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location = 'dashboard_komentar.php';
                            }
                        });
                    } else {
                        Swal.fire({
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Gagal mengubah komentar: ' + xhr.responseText,
                        icon: 'error',
                        confirmButtonText: 'Tutup'
                    });
                }

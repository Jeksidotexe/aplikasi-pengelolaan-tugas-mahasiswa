...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $dataKategori = $kategori->getKategoriById($id);
        if (!$dataKategori) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_kategori)) {
        ...
        // Jika validasi berhasil, lanjutkan untuk mengubah kategori
        $kategori = new Kategori($db, $id, $nama_kategori);

        try {
            if ($kategori->editKategori($id, $nama_kategori)) { // Pastikan metode ini ada di model Kategori Anda
...
include 'Views/Forms/form_editKategori.php';
